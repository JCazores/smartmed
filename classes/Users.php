	...
	public function save_users() {
		...
		if (isset($password) && isset($confirm_password)) {
			if ($password !== $confirm_password) {
				echo json_encode(['status' => 5, 'message' => 'Passwords do not match.']);
				exit();
			}
		}
		...
		if (isset($oldpassword)) {
			if (md5($oldpassword) != $this->settings->userdata('password')) {
				echo json_encode(['status' => 4, 'message' => 'Old password is incorrect.']);
				exit();
			}
		}
		...
		if ($chk > 0) {
			echo json_encode(['status' => 3, 'message' => 'Username already exists.']);
			exit();
		}
		...
		// Save password if it's provided
		if (!empty($password)) {
		...
		if (empty($id)) {
			...
			if ($qry) {
				...
				echo json_encode(['status' => 1, 'message' => 'User details successfully saved.']);
			} else {
				echo json_encode(['status' => 2, 'message' => 'Failed to save user details.']);
			}
		} else {
			...
			if ($qry) {
				...
				echo json_encode(['status' => 1, 'message' => 'User details successfully updated.']);
			} else {
				echo json_encode(['status' => 2, 'message' => 'Failed to update user details.']);
			}
		}
	
		// Handle avatar upload
		if (isset($_FILES['img']) && $_FILES['img']['tmp_name'] != '') {
			// Avatar upload code here...
		}
	
		exit();  // Ensure script execution ends after response is sent
	}
